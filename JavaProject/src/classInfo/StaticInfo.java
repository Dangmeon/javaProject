...
public class StaticInfo {
	...
	static void s2(int x) {f1(3);} // static 메소드에서 인스턴스 메서드 호출 - 호출 불가능
	static void s3(int x) {b = x;} // static 메소드에서 static 필드 사용
	static void s4(int x) {s3(3);} // static 메소드에서 static 메서드 호출
...
class Television {
	...
	static {
		// 단순 값을 저장하는 초기화가 아닌 복잡한 연산을 진행하는 경우 -> static 초기화 블록 사용
		info = company + "-" + model;
	}
	{
...
class Test10 {
	...
	// static 은 this 사용 불가능
	static void Method2() {
		this.a = 10; // this 는 인스턴스 참조하기 때문에 static 에서는 사용 불가능
		// static 메소드에서 인스턴스 필드 또는 인스턴스 메소드 사용 하려면 인스턴스 생성 후 사용
