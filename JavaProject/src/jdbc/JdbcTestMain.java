...
import java.sql.Statement; // 쿼리 구문 저장할 공간 생성 (Connection 객체 통해 필요한 객체 생성)
	...
	public static void main(String[] args) {
		...
		if(con == null) { // DB 접속 실패
			...
			try {
				...
				// 쿼리를 저장할 객체를 하나 생성 : Statement 객체 생성
				stmt = con.createStatement();

				// 쿼리를 DBMS에 전달, 실행 요청, 실행된 결과 반환 : 반환이 성공하면 조회된 결과가 rs의 참조로 반환
				rs = stmt.executeQuery(query); // select 구문일 때 사용하는 메서드, ResultSet 반환

				System.out.println("----------------전체 도서 정보 조회-------------------");
				System.out.println("도서번호 \t 도서명 \t\t\t\t 저자 \t\t 가격 \t\t 발행일   \t 재고 \t 출판사 번호");

				// ResultSet 객체 활용 포인터 이동해가면서 각 속성 값 추출
				// getXXX(속성번호) : XXX -> String, Integer, Double, Date
				while (rs.next()) {
					// 각 메소드가 속성값 1개를 추출, 전체 메소드를 실핼하면 1개 레코드의 각 속석 데이터가 변수에 저장됨
