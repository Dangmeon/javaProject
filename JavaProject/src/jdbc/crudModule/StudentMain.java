...
public class StudentMain {
	 ...
	 *  
	 *  getStdInfo(Scanner) 메서드에 대해 insert 와 update 에 대해 학번 입력을 다르게 진행하도록 수정
	 *  insert(학번 입력받기) update(학번 입력 없음)
	 *  
	 * */
	...
	public static void main(String[] args) {
		...
		IStudentDAO dao = new StudentDAO(); // 객체 생성시 자동으로 DB연결
		...
		while(true) {
			...
			switch(selMenu) {
			case "1":
				dao.insertStudent(ReadWrite.getStdInfo(sc, true));
				break;
			case "2":
				System.out.println("학번을 입력하세요");
				stdNo = sc.nextLine();
				ReadWrite.writeStdInfo(dao.detailStudent(stdNo)); // 기존 저장 정보 출력
				dao.updateStudent(ReadWrite.getStdInfo(sc, false)); // update 인 경우 학번 전달해서 학번 입력없이 진행되도록 수정
				break;
			case "3":
				System.out.println("학번을 입력하세요");
				stdNo = sc.nextLine();
