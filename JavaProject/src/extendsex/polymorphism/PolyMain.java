...
public class PolyMain {
	...
	public static void main(String[] args) {
		...
		System.out.println("Cat 타입 인스턴스가 Anomal 참조변수로 대입되면서 형변환 발생");
		a.show(); 
		a.sound();

		// 다형성이 진행된 걍우 부모클래스 참조변수는 부모클래스의 멤버만 접근할 수 있지만
		// 메소드가 재정의된 경우 자식클래스 메소드를 사용하게 됨
		a = new Cat2(); // Cat2 는 show 가 재정의 되지 않으므로 Animal 메소드가 호출됨
		a.show();
		// a.show1(); // Cat2 인스턴스가 대입이 되었지만 Cat2 인스턴스의 일반멤버는 사용 불가능 재정의 되지 않았다면 부모 클래스 멤버만 사용 가능
		...
		ap = new Cat(); // 객체 치환이 일어나는 경우 참조하던 객체 참조를 잃어버림
		// 어떤 변수도 참조하지 않는 인스턴스는 가비지가 되어서 jvm 에 의해 자동으로 수거
		...
		// 자동 형변환 예시
		ap = new Cat(); // sub 클래스 인스턴스 타입이 대입되면서 super class 타입으로 자동 형변환
		ap.show();
		ap.sound();
		...
		// 강제 형변환 예시
		// Dog 객체를 참조하는 변수는 ap1 만 있다고 가정 - Dog 객체의 일반 메서드는 사용 불가능
		// 단, ap1 의 참조주소는 Dog 개게 주소이므로 강제 형변환 가능
		Dog d1 = (Dog)ap1;
		// d1 과 ap1 은 같은 인스턴스 참조 (단, 성격은 다름)
		d1.dogMethod();
