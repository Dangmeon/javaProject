...
public class MergeSort { // 정렬을 위한 공간이 하나 더 필요한 정렬
	...
	private static void mergeSort(int[] a, int left, int right) {
		...
		System.out.print("왼쪽 부분 리스트 (index : " + left + " ~ " + mid + ") : ");		
		...
		System.out.print("오른쪽 부분 리스트 (index : " + (mid + 1) + " ~ " + right + ") : ");	
	...
	private static void merge(int[] a, int left, int mid, int right) {
		...
		// 각 부분리스트의 끝점을 지나지 않는 동안 반복
		while(l <= mid && r <= right) {
			/*
			 * 	왼쪽 부분리스트의 첫 번째 원소 값이 더 작으면 임시 배열에 왼쪽 부분리스트의 원소를 저장
			 * 	정렬이 종료된 원소 다음 원소로 l++, 임시 배열 idx 도 ++
			 * */
			if(a[l] <= a[r]) {
				sorted[idx] = a[l];
				idx++;
				...
				/*
				 * 	오른쪽 부분리스트의 첫 번째 원소 값이 더 작으면 임시 배열에 오른쪽 부분리스트의 원소를 저장
				 * 	정렬이 종료된 원소 다음 원소로 r++, 임시 배열 idx 도 ++
				 * */
				sorted[idx] = a[r];
				idx++;
				r++;
		 ...
		 * */
		if(l > mid) {
			while(r <= right) { // 오른쪽 리스트에 원소가 남아있는 동안
					sorted[idx] = a[r];
					r++;
					idx++;
			}
		 ...
		 * */
			...
			while(l <= mid) { // 오른쪽 리스트에 원소가 남아있는 동안
				sorted[idx] = a[l];
				l++;
				idx++;
			}
		...
		/*
		 * 	임시 배열에 정렬된 결과를 기존의 배열에 복사하여 옮겨준다
		 * */
