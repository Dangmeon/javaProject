...
public class LottoStore {
	...
	public void getLotto(int money) {
		...
		for(int i = 1; i <= num; i++) { // 로또 장 수
			while(lottoSet.size()<6) { // 로또 번호 생성
				lottoSet.add((int)(Math.random()*45 +1)); // 1 ~ 46 ㅂㅁ위의 숫자응 set 에 저장, 동일 숫자는 저장되지 않음
			}
			ArrayList<Integer> lottoList = new ArrayList<>(lottoSet); // ArrayList 생성자에는 ArrayList로 변환가능한 객체
	...
	public static void main(String[] args) {
		new LottoStore().lottoStroreStart();
	}
