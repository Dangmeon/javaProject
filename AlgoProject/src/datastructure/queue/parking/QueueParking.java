	...
	public static void parkingMenu() {
		...
		MENU : while(true) {
			...
			case "0":
				...
				break;
			case "1":
				System.out.println("차량의 고유값을 입력하세요. : ");
				char carChr= sc.nextLine().charAt(0);
				
				if(parking.contains(Character.toLowerCase(carChr))== -1) {
					if(parking.enqueue(Character.toLowerCase(carChr))) {
			...
			case "2":
				 ...
				 * 	2 - 2. 1시간 미만 : 출차 불가
				 * 	2 - 3. 1시간 이상 : 출차 - 선행차량 dequeue 후 다시 enqueue 가 모두 완료되면
				...
				if(parkChar != 'E') {
					if(Character.toLowerCase(chr1) == parkChar) {
						...
						System.out.println("아직 출차할 수 없습니다. 주차 시간이 1시간이 지난 경우 출차할 수 있습니다. 주차 시간이 1시간 이상이면 1"
								+ "1시간 미만이면 0을 입력하세요.");
						int time = sc.nextInt();
						sc.nextLine();
						if(time >= 1) {
							...
							if(num1 >= 0 ) {
								for(int i = 1; i <= num1; i++) {
									if(parking.isFull()) {
										System.out.println("주차장에 이동 공간이 없어 선행 차량 이동이 불가능하여 미리 출차가 불가합니다. 대기해주세요.");
										continue MENU;
									}else {
								...
								parking.dequeue();
								System.out.println("선행 차량 이동 후 출차 완료");
							}else {
