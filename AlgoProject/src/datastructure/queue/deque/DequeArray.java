...
// java.util.Dequr 인터페이스를 ArrayDeque 로 구현
// 배열 기준이기 때문에 용량 초과 overflow 발생가능
public class DequeArray {
	
	public static void main(String[] args) {
		//Deque<String> dq = new ArrayDeque<>();
		
		Deque<String> dq = new LinkedList<>(); // LinkedList 도 Deque 인터페이스 구현 객체
		...
		// Deque addFirst() : 앞쪽에 삽입
		System.out.println("\n앞 쪽에 삽입"); // addFirst
		dq.addFirst("바나나");
		System.out.println(dq);
		dq.add("복숭아");
		System.out.println(dq);
		System.out.println("\n뒤 쪽에 삽입"); // addLast, add 와 동일
		dq.addLast("복숭아");
		System.out.println(dq);
		...
		// remove(값) : 해당 값이 deque 에 있으면 true 반환하고 해당값의 원소를 삭제
		System.out.println("\nremove : " + dq.remove("사과")); 
		System.out.println(dq);
		
		// remove(값) : 해당 값이 deque 에 없으면 false 반환
		System.out.println("\nremove : " + dq.remove("사과")); 
		System.out.println(dq);
		
		// remove(값) : 해당 값이 deque 에 있고 동일 값이 여러개 있는 경우 첫 번째 값을 삭제
		...
		// addFirst, pollFirst : front 의 삽입과 삭제
		// addLast, pollLast : rear 의 삽입과 삭제
		
		// 데크를 스택처럼 사용 가능 : pop/push
