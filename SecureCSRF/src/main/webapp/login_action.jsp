...
try
{
	...
	String db_address = "jdbc:oracle:thin:@localhost:29889/freepdb1";
    ...
    if(result.next()){
		...
		// 연결되어 있는 동안 객체는 활성화됨, 특정 조건에 만족하지 않으면 객체는 제거됨 -> 클라이언트와의 연결이 종료
		// 클라이언트 요청이 들어오면 세션 객체는 생성됨 -> 클라이언트에게 응답시 쿠크에 session id 값을 전송함
		// 클라이언트 재요청시에 session id 값을 서버측으로 전달해야함 -> 전달된 session id 를 활용 로그인 여부를 어플리케이션은 확인하게 됨
