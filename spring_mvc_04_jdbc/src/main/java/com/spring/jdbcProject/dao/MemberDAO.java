...
public class MemberDAO {
	...
	// 반환 결과를 담을 DTO 필드를 추가 - 지동 주입되는 객체 공통 사용 객체(한 번 들어오고 변경되지 않음)
	// 의존성 주입은 객체 생성될 때 한 번 주입되기 떄문에 동적으로 계속 객체가 생성되는 경우 사용 불가능
	...
	public MemberDAO(DataSource dataSource) {
		try {
			this.dataFactory = dataSource; // dbcp 연동
		}catch(Exception e) {
			e.printStackTrace();
		}
	}
	
	// 회원 정보 조회 : 전체 회원 select 해서 반환
	public ArrayList<MemberDTO> memberSelect(){
		...
		try {
			...
			while(rs.next()) {
				String memId = rs.getString("memId"); // 일반 변수 / 데이터 컬럼명
				...
				// MemberDTO 에 담아서
				// dto 객체 인스턴스 자동 주입 가능? -> 어노테이션 이용해서 불가능
