 ...
 * Gugudan 클래스와 Rect 클래스의 핵심 기능 메소드 수행 전/후에 시간 출력
 * */

// 공통 기능 메소드 포함(proxy 의 절차를 구성하게 됨)
@Aspect
public class PerformanceAspect {
	// where : 어디에 적용할 것인지 (annotation 패키지 내 모든 클래스)
	@Pointcut("within(com.spring_aop.annotation.*)")
	private void pointcutMethod(){ // annotation 을 부착하고 이름을 사용하기 위한 용도 -> 빈 메소드 구성
		
	}
	// when : 핵심기능 메소드 전에 시작하고 핵심기능 위임
	@Around("pointcutMethod()")
	public Object trace(ProceedingJoinPoint joinPoint) throws Throwable{
		Signature s = joinPoint.getSignature(); // joinPoint 의 정보 추출
		String methodName = s.getName(); // 핵심기능 메소드의 이름 추출
