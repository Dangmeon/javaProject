	...
	<form action="post_read.jsp" method="get">
		...
		<table border="1">
			...
			<tr>
				...
				<td><a href="post_read.jsp?num=${num }"><c:out value="${title }"/>
				</a></td>
				<td><c:out value="${regdate }"/></td>
				<td>
					<button type="button" value="수정"
						onClick="location.href='post_modify.jsp?num=${num }'">수정</button>
					<button type="button" value="삭제"
						onClick="location.href='post_delete_send.jsp?num=${num }'">삭제</button>
				</td>
			</tr>
			<% }%>
		</table>
...
<%-- <td><%=result.getInt("num") %></td>
				...
				<td><a href="post_read.jsp?num=<%=result.getInt("num")%>"> <%=result.getString("title") %>
				</a></td>
